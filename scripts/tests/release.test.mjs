...
test('every valid explicit version is eligible for release', () => {
  assert.equal(shouldCreateRelease({ major: 1, minor: 5, patch: 0 }, false), true);
  assert.equal(shouldCreateRelease({ major: 1, minor: 4, patch: 9 }, false), true);
  assert.equal(shouldCreateRelease({ major: 1, minor: 10, patch: 1 }, false), true);
